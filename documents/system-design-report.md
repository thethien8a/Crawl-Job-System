...
│ ready_jobs for frontend      │ │ jobs_v2          │ │ star schema BI   │
└──────────────────────────────┘ └────┬────────────┘ └────────┬─────────┘
                                      │                       │
                         ┌────────────▼────────────┐ ┌────────▼─────────┐
...
| Recommendation stack | Qdrant Cloud, OpenRouter Embedding, FastAPI, React | Index vector job, nhận CV, tính gợi ý và hiển thị Top 10 |
| Cloud services | AWS S3, Supabase, MotherDuck, Qdrant Cloud, OpenRouter | Lưu Bronze/Silver/CV, phục vụ OLTP, phục vụ OLAP, phục vụ vector search/embedding |
...
Build job text = clean_job_title + requirements_cleaned + job_description_cleaned
      ↓
OpenRouter embedding, input_type=search_document, 768 dimensions
      ↓
Qdrant upsert collection jobs_v2, id=UUID5(job_url), payload=job metadata + require_*
```
...
| Job input type | search_document |
| CV input type | search_query |
...
Clean text + trích kỹ năng bằng taxonomy Google Sheets/CSV
      ↓
OpenRouter embedding, input_type=search_query
      ↓
...
| Qdrant Cloud | Vector database cho jobs_v2 | QDRANT_URL, QDRANT_API_KEY, QDRANT_COLLECTION |
| OpenRouter | Embedding job/CV | OPENROUTER_API_KEY, OPENROUTER_BASE_URL, EMBEDDING_MODEL, EMBEDDING_DIM |
...
| Qdrant/OpenRouter | QDRANT_URL, QDRANT_API_KEY, QDRANT_COLLECTION, OPENROUTER_API_KEY, OPENROUTER_BASE_URL, EMBEDDING_MODEL, EMBEDDING_DIM |
...
| Qdrant | Vector search trên collection jobs_v2, filter payload cho location/experience/deadline |
| OpenRouter embedding | Batch embedding khi index job, chỉ embed 1 CV ở request time |
