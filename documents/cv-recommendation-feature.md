...
| E1/J3 | Embedding | Dùng **OpenRouter Embeddings API**; mặc định `openai/text-embedding-3-small`, output 768 chiều |
...
│   3. embed (OpenRouter, batch) ┼──┐         │  3. extract skills (taxonomy GSheet)│
│   4. upsert + query-time filter │  │         │  4. embed CV (OpenRouter, query)  │
...
| Vector dimension | `768` (`dimensions=768`, **L2-normalize** sau khi nhận vector) |
| Distance metric | `Cosine` |
| Qdrant collection name | `jobs_v2` |
| Input type khi embed JOB | `search_document` |
| Input type khi embed CV | `search_query` |
...
Collection: jobs_v2
  vectors:
    size: 768
    distance: Cosine
  point id: UUID5(namespace=URL, name=unique_url) # idempotent upsert theo logical job
  payload:
    unique_url         : keyword         # khóa dedup/upsert nội bộ
    job_url            : keyword
    job_title          : text
    ...
    silver_date_ts     : integer         # epoch giây; retention dữ liệu 30 ngày gần nhất
    indexed_at_ts      : integer         # epoch giây; debug/fallback cleanup
...
Tạo **payload index** cho `unique_url`, `job_url`, `source_site`, `clean_location`, `min_exp_level`, `max_exp_level`, `deadline_ts`, `silver_date_ts`, `indexed_at_ts` để filter/delete nhanh.
...
src/storage_layer/Qdrant/
  config.py                       # QDRANT_URL/API_KEY, COLLECTION, DIM, OPENROUTER_API_KEY, model name
  client.py                       # get_qdrant_client() -> QdrantClient (single factory)
  schema.py                       # constants + IndexSettings/IndexContext
  silver.py                       # đọc Silver, project columns, dedup theo unique_url
  embedder.py                     # embed_documents(texts) -> vectors (OpenRouter)
  payload.py                      # dựng document text, point id, payload, timestamp
  collection.py                   # tạo collection/payload index, xóa retention
  scripts/index_silver_to_qdrant.py   # CLI orchestration: Silver -> embed -> upsert -> cleanup
```
...
4. **Embed theo batch** qua OpenRouter `/embeddings`. Dùng `input_type=search_document`, `dimensions=768`, sau đó **L2-normalize**.
5. Dựng `PointStruct(id=uuid5(unique_url), vector=v, payload=QdrantJobPayload(...))`; `job_url` vẫn giữ trong payload để trả link gợi ý cho user.
6. `client.upsert(collection, points)` theo lô (vd 100 điểm/lần).
7. Xóa hard points có `silver_date_ts` cũ hơn retention window (mặc định 30 ngày); `deadline_ts` vẫn chỉ dùng để lọc job hết hạn ở query-time (xem 7.4).
...
- File mới `src/orchestration_layer/dags/qdrant_index.py`, dùng **DockerOperator** chạy `python -m src.storage_layer.Qdrant.scripts.index_silver_to_qdrant` trong container `lakehouse-crawler` (đúng pattern hiện tại — DAG chỉ orchestrate).
- Lịch: cùng cadence với Supabase load; params mặc định index lại rolling window 3 ngày gần nhất và cleanup retention 30 ngày.
- Params override qua UI: `{"from_date": "...", "to_date": "...", "retention_days": 30}`.

### 6.5. Dependencies thêm (requirements Lakehouse-Lite)

`qdrant-client`, `openrouter` (SDK OpenRouter).
...
EMBEDDING_DIM=768
DOCUMENT_EMBEDDING_INPUT_TYPE=search_document
```
...
backend/app/services/
  ...
  embedder.py      # embed_query(text) -> vector (OpenRouter, input_type=search_query, dim 768, normalize)
...
5. `embedder`: embed text CV (1 vector, `input_type=search_query`).
...
- **Lưu ý provider embedding:** dữ liệu CV/job được gửi qua OpenRouter và provider phía sau; ghi rõ trong điều khoản nếu demo công khai.

### 7.7. Dependencies thêm (requirements backend)

`qdrant-client`, `openrouter`, `pdfplumber`, `python-docx`, `flashtext`, `polars` (nếu port taxonomy dùng polars) hoặc thay bằng `pandas`/`csv` cho nhẹ. `boto3` chỉ cần nếu bật optional lưu CV lên S3 sau MVP.
...
2. **OpenRouter:** tạo API key → `OPENROUTER_API_KEY`; đảm bảo backend và Lakehouse dùng cùng model/dimension/collection.
...
| Rate-limit OpenRouter/provider | Index/định kỳ chậm/lỗi | Batch + backoff; chọn provider/model phù hợp ngân sách |
...
1. **M1 — Hạ tầng:** tạo Qdrant Cloud, OpenRouter key, chốt SHARED CONTRACT. S3 lưu CV là optional sau MVP.
