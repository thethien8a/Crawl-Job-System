...
Silver (S3) ──→ Embed (Gemini) ──→ Qdrant (Vector DB) ──→ CV Recommendation
                                         ↑
                                    CV Upload ──→ Parse + Embed (Gemini)
```
...
| **Xử lý dữ liệu** | Polars, FlashText, RapidFuzz, Great Expectations, gspread, Gemini Embedding, pdfplumber, python-docx | Làm sạch, biến đổi, xác minh, embedding dữ liệu |
...
> Các công nghệ xử lý dữ liệu bao gồm làm sạch và biến đổi (Polars), trích xuất nhãn taxonomy (FlashText), khớp chuỗi mờ (RapidFuzz), xác minh chất lượng (Great Expectations), quản lý taxonomy seed (gspread), embedding ngữ nghĩa (Gemini Embedding), và trích xuất text từ CV (pdfplumber, python-docx).
...
### 4.6 Google Gemini Embedding

| Thuộc tính | Chi tiết |
|-------------|----------|
| **Model** | `gemini-embedding-001` |
| **Dimension** | 768 (MRL, L2-normalized) |
...
**Mục đích:** Google Gemini Embedding tạo vector biểu diễn ngữ nghĩa cho văn bản tuyển dụng và CV. Job text được embed với `task_type=RETRIEVAL_DOCUMENT`, CV text với `task_type=RETRIEVAL_QUERY`. Vector 768 chiều được L2-normalize sau khi cắt (MRL) để đảm bảo cosine similarity chính xác.
...
- **MRL (Matryoshka Representation Learning):** Cho phép cắt vector xuống 768 chiều mà vẫn giữ chất lượng tìm kiếm, giảm chi phí lưu trữ trên Qdrant.
- **Hai task type:** Phân biệt `RETRIEVAL_DOCUMENT` (cho job) và `RETRIEVAL_QUERY` (cho CV) tối ưu hóa kết quả tìm kiếm ngữ nghĩa.
...
| **Xử lý dữ liệu** | Google Gemini Embedding | gemini-embedding-001 | Xử lý | Mô hình embedding đa ngôn ngữ (768 chiều) |
...
│                                                                     │
│  ┌──────────────┐    ┌──────────────────┐    ┌──────────────┐     │
│  │ Silver S3    │───▶│ Gemini Embedding  │───▶│   Qdrant     │     │
...
### 11.8 Tại sao chọn Qdrant + Gemini Embedding cho gợi ý CV?
...
- **Gemini embedding đa ngôn ngữ:** `gemini-embedding-001` hỗ trợ tốt cả tiếng Anh và tiếng Việt, phù hợp thị trường tuyển dụng Việt Nam mà không cần huấn luyện mô hình riêng.
- **MRL dimension cắt:** Gemini hỗ trợ Matryoshka Representation Learning cho phép cắt vector xuống 768 chiều, giảm 50% chi phí lưu trữ trên Qdrant so với vector 1536 chiều đầy đủ mà vẫn giữ chất lượng tìm kiếm.
