...
| CV Recommendation Layer | Trong phạm vi triển khai | Gợi ý Top 10 việc làm theo CV bằng OpenRouter embedding, Qdrant và hybrid scoring |
| Qdrant/OpenRouter indexing | Trong phạm vi triển khai | Index dữ liệu Silver sang Qdrant collection jobs_v2 bằng vector 768 chiều |
...
| FR-R02 | Lưu vector và payload job | Vector 768 chiều được upsert vào Qdrant collection jobs_v2, payload giữ job_url, title, company, location, salary, experience, source_site và require_* |
...
Silver S3 Parquet
        ...
        ├──→ OpenRouter Embedding → Qdrant jobs_v2 phục vụ gợi ý CV
...
| Qdrant indexing | Silver Parquet | Ghép text job, embed bằng OpenRouter, upsert vector/payload | Collection jobs_v2 |
...
Tầng gợi ý CV sử dụng Qdrant collection jobs_v2 làm chỉ mục vector cho các tin tuyển dụng còn hiệu lực. Mỗi điểm vector đại diện cho một job_url và được tạo từ nội dung đã làm sạch ở Silver.
...
Vector job dùng mô hình openai/text-embedding-3-small qua OpenRouter, dimension 768 và distance metric Cosine. Point id được sinh ổn định theo job_url để việc upsert hằng ngày không tạo trùng.
