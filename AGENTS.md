...
```bash
cp .env.example .env          # then fill ITVIEC_USERNAME/PASSWORD, AWS keys
# Pre-create bronze + silver buckets in the AWS S3 console (names from src/storage_layer/MinIO_S3/config/bucket.yml)
...
- AWS S3: `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` (default `ap-southeast-1`)
- `load_dotenv()` reads `.env` at import time; pipeline Docker containers receive the same `.env` via bind mount
- ITviec crawler **requires** `ITVIEC_USERNAME` / `ITVIEC_PASSWORD`
- Airflow DAGs require `FERNET_KEY`, `WEBSERVER_SECRET_KEY`, `HOST_REPO_PATH`, `PIPELINE_IMAGE`
...
2. **Bronze upload** (`bronze.main --source <site>` or all) gz-compresses JSONL, uploads to S3 `bronze/<source>/jobs/year=.../`, then **clears `temp_data/`**
...
- One `docker-compose.yaml` for Airflow (LocalExecutor, DockerOperator); object storage is AWS S3 (no local container)
...
| Service | URL |
|---------|-----|
| AWS S3 | `https://s3.<region>.amazonaws.com` (managed) |
